package com.cheng.appstore.views;
...
public class RandomLayout extends ViewGroup {
    ...
    private void init() {
        mLayouted = false;
        mRdm = new Random(System.currentTimeMillis());
        setRegularity(10, 10);
            ...
            if (!mFixedViews.contains(child)) {// mFixedViews用于存放已经确定好位置的View，存到了就没必要再次存放
                ...
                while (availAreaCount > 0) { // 如果使用区域大于0，就可以为子View尝试分配
                    ...
                    if (mAreaDensity[row][col] < areaCapacity) {// 区域密度未超过限定，将view置入该区域
                        ...
                        // 确定左边，等于区域宽度*左边的区域
                        params.mLeft = getPaddingLeft() + (int) (colW * col + mRdm.nextInt(xOffset))+25;
                        int rightEdge = contentRight - childW - 20;
                        if (params.mLeft > rightEdge) {// 加上子View的宽度后不能超出右边界
                            params.mLeft = rightEdge;
                        }
                        params.mRight = params.mLeft + childW;

                        params.mTop = getPaddingTop() + (int) (rowH * row + mRdm.nextInt(yOffset))+15;
                        int bottomEdge = contentBottom - childH - 70;
